...
//	public static final String FILE_NAME = "/home/tao/software/station/tianchi/engine/engine_java/src/test/java/io/mappedbus/MappedBusWriterTest";
...
//		new File(FILE_NAME).delete();
	...
	@Test public void testWriteMemoryKey() throws Exception {
		MappedBusWriter writer = new MappedBusWriter(FILE_NAME, FILE_SIZE, RECORD_SIZE, true);
		writer.open();
		...
		MemoryKey memoryKey = new MemoryKey(key, offset);
		writer.write(memoryKey);

		num = 1;
		offset = num | (valueLogNo << 24);
		memoryKey = new MemoryKey(key, offset);
		writer.write(memoryKey);
