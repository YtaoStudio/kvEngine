...
public class MappedBusReader {
	...
	public boolean next(int pos) throws EOFException {
		limit += pos;
		if (limit >= fileSize) {
			throw new EOFException("End of file was reached");
		}
	...
	public int readType(int pos) {
		typeRead = true;
		limit += Length.StatusFlags + pos;
		int type = mem.getInt(limit);
		limit += Length.Metadata;
		return type;
	}
	...
	public MappedBusMessage readMessage(MappedBusMessage message, int pos) {
		if (!typeRead) {
			readType(pos);
		}
		typeRead = false;
	...
	public int readBuffer(byte[] dst, int offset, int pos) {
		limit += Length.StatusFlags;
		int length = mem.getInt(limit);
		limit += Length.Metadata;
		mem.getBytes(limit, dst, offset, length);
